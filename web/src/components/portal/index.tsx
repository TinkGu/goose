...
export type PortalFcChildren = (onDestroy: () => void) => ReactNode;
export type PortalChildren = ReactNode | PortalFcChildren;

const noop = () => null;

export interface PortalProps {
  children?: ReactNode | PortalFcChildren;
  /** children 的别名，仅在命令式调用下有效 */
  content?: ReactNode | PortalFcChildren;
  /** 销毁时触发 */
  onDestroy?: () => void;
}
 ...
 * 浮层主体内容
 */
function BasePortal({ children = null, onDestroy = noop }: PortalProps) {
  if (typeof children === 'function') {
    return <>{(children as PortalFcChildren)(onDestroy)}</>;
  }
  return <>{children}</>;
}
